...
## Définition
L’**OCP** promeut des **points de variation protégés** : on conçoit une **abstraction stable** (interface, classe abstraite) que l’on **étend** par de **nouvelles implémentations** plutôt que d’**éditer** le code client. En pratique, cela se traduit souvent par le patron **Stratégie** : la classe consommatrice **compose** une politique de calcul, qu’on peut **substituer** sans toucher à la classe elle‑même. Cette approche **réduit** les *switch* ou *if/else* qui s’allongent, **protège** le code testé et **facilite** l’ajout de fonctionnalités. Couplé au **DIP** (dépendre d’interfaces) et au **LSP** (garantir la substituabilité), l’OCP rend l’application **extensible** et **robuste**.
